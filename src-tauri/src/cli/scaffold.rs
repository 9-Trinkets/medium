...
pub fn run(name: &str, path: Option<&str>) -> Result<()> {
    ...
    let path = if let Some(p) = path {
        ...
        fs::create_dir_all(&ghosts_path).with_context(|| {
            format!(
    ...
    fs::create_dir_all(&animations_dir).with_context(|| {
        format!(
            "Failed to create animations directory: {}",
...
fn create_placeholder_png() -> Vec<u8> {
    ...
    vec![
        ...
        0x08, 0x06, 0x00, 0x00,
        0x00, // bit depth 8, color type 6 (RGBA), compression/filter/interlace
        0x1F, 0x15, 0xC4, 0x89, // CRC
        // IDAT chunk (minimal)
        0x00, 0x00, 0x00, 0x0A, // chunk length
        0x49, 0x44, 0x41, 0x54, // "IDAT"
        0x78, 0x9C, 0x62, 0xF8, 0xCF, 0xC0, 0x00, 0x00, 0x03, 0x01, 0x01, 0x00, 0x18, 0xDD, 0x8D,
        0xB4, // CRC
        // IEND chunk
